...
export default async function Home() {
  ...
  return (
    <main className="relative w-screen h-screen flex flex-col items-center justify-center overflow-hidden">
      <h1 className="fixed bottom-5 right-5">made with ☕ by nabeel</h1>
      <Image src={logo} alt="logo" className="size-45" />
      <SignInButton>
        <button className="relative inline-flex items-center justify-center px-10 py-3 overflow-hidden tracking-tighter text-white bg-black mt-3 rounded-md group active:scale-95 transition-all duration-250 cursor-pointer">
          <span className="absolute w-0 h-0 transition-all duration-500 ease-out bg-primary rounded-full group-hover:w-56 group-hover:h-56" />
          ...
          <span className="relative text-xl font-semibold">Sign in to K</span>
        </button>
      </SignInButton>
    </main>
  )
